...
/**
 * Neutralise HTML before the markdown pass turns text into markup.
 *
 * The welcome template renders through `dangerouslySetInnerHTML` after a few
 * regex replacements, and the message content went in raw — so anything that
 * reached `message.content` containing `<img src=x onerror=…>` executed in the
 * reader's session. Message content is not trustworthy: it is either typed by
 * a person or generated by a model, and a model will emit whatever the text it
 * was given asks it to.
 *
 * Escaping has to happen *first*. Doing it after the replacements would undo
 * the `<strong>` tags the pass just wrote.
 */
...
const ChatMessage = ({ message, isLastMessage, onActionClick }) => {
  ...
  // See `escapeHtml` below — the markdown pass writes raw HTML, so the text
  // has to be neutralised before it, not after.
